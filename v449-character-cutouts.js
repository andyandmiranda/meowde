(function applyMeowdeV450CharacterImages(){
  ...
  function decorateHero(){
    ...
    if(!scene){
      const current=hero.querySelector(".v448-approved-hero,.v444-coding-scene,.v435-coding-scene"),label=english()?"Meowde learning with headphones":"헤드폰을 쓰고 학습하는 Meowde";
      const markup=`<div class="v444-coding-scene v448-approved-hero v449-coding-scene">${imageMarkup("coding","",label)}</div>`;
      if(current)current.outerHTML=markup;else (hero.querySelector(".hero-main")||hero).insertAdjacentHTML("afterbegin",markup);
      scene=hero.querySelector(".v449-coding-scene");
    }
    scene.querySelectorAll(".v449-character").forEach((node,index)=>{if(index)node.remove()});
    const character=scene.querySelector(".v449-character");if(character)updateImage(character,"coding","",english()?"Meowde learning with headphones":"헤드폰을 쓰고 학습하는 Meowde");
  }
  function decorateBrand(){const mark=document.querySelector(".brand-mark");if(mark)mark.innerHTML=imageMarkup("base","v448-brand-cat","Meowde")}
  function level(){const current=window.MeowGrowth&&typeof MeowGrowth.stage==="function"?MeowGrowth.stage():null,stages=window.MeowGrowth&&Array.isArray(MeowGrowth.stages)?MeowGrowth.stages:[];return Math.max(1,stages.findIndex(item=>current&&item.id===current.id)+1)}
  ...
  function decorateTrail(){document.querySelectorAll(".trail-cat").forEach(slot=>{slot.innerHTML=imageMarkup("smug","",english()?"Confident Meowde":"자신만만한 Meowde")})}
  function title(card){const heading=card&&card.querySelector("h3");return heading?heading.textContent.trim().toLowerCase():""}
  function addCardPose(card,pose,label){if(!card)return;card.classList.add("v449-decorated-card");const current=card.querySelector(":scope > .v449-card-pose");if(current)updateImage(current,pose,"v449-card-pose",label);else card.insertAdjacentHTML("beforeend",imageMarkup(pose,"v449-card-pose",label))}
  ...
  function accuracy(reward){const value=reward&&reward.querySelector(".stats3 .stat3:nth-child(3) b"),match=value&&String(value.textContent||"").match(/\d+/);return match?Number(match[0]):100}
  function decorateFeedback(){
    const reward=document.querySelector(".reward-screen");
    if(reward){const value=accuracy(reward),pose=value>=85?"happy":value>=65?"smug":"meh",current=reward.querySelector(".v449-character,.meowde-approved-character,svg,img");if(current)updateImage(current,pose,"v449-feedback-character",english()?"Meowde result":"학습 결과 Meowde");else reward.insertAdjacentHTML("afterbegin",imageMarkup(pose,"v449-feedback-character",english()?"Meowde result":"학습 결과 Meowde"))}
    const feedback=document.querySelector(".feedback");
    if(feedback){const pose=runtimeError()?"error":feedback.classList.contains("ok")?"happy":"surprised",current=feedback.querySelector(".v449-feedback-character");if(current)updateImage(current,pose,"v449-feedback-character","Meowde");else feedback.insertAdjacentHTML("afterbegin",imageMarkup(pose,"v449-feedback-character","Meowde"))}
  }
  function setProfilePose(selector,pose,label){const slot=document.querySelector(selector);if(!slot)return;const current=slot.querySelector(":scope > .v449-character,:scope > .meowde-approved-character,:scope > svg,:scope > img");if(current)updateImage(current,pose,"v449-profile-pose",label);else slot.insertAdjacentHTML("afterbegin",imageMarkup(pose,"v449-profile-pose",label))}
  function decorateProfile(){
    document.querySelectorAll(".v420-avatar").forEach(slot=>{const current=slot.querySelector(":scope > .v449-character,:scope > .meowde-approved-character,:scope > svg,:scope > img"),label=english()?"Meowde with headphones":"헤드폰을 쓴 Meowde";if(current)updateImage(current,"music","v449-profile-pose",label);else slot.insertAdjacentHTML("afterbegin",imageMarkup("music","v449-profile-pose",label))});
    setProfilePose(".profile-card","coding",english()?"Meowde learning with headphones":"헤드폰을 쓰고 학습하는 Meowde");
    setProfilePose(".v415-profile-growth .growth-avatar","smug",english()?"Confident Meowde":"안경을 쓴 자신만만한 Meowde");
  }
