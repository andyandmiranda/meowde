(function applyMeowdeV428Events(){
  ...
  function seasonalCard(){
    ...
    return `<section class="card v428-season-card" data-season="${current.accent}"><div class="v428-season-head"><span>${current.icon}</span><div><div class="section-kicker">${ko?"시즌 이벤트":"Seasonal event"}</div><h3>${ko?current.titleKo:current.titleEn}</h3><p>${ko?"학습 5회를 완료하고 한정 배지를 받으세요.":"Complete 5 learning sessions to earn a limited badge."}</p></div></div><div class="v428-event-track"><i style="width:${percent}%"></i></div><div class="v428-event-foot"><b>${value}/${current.goal}</b><span>🎁 ${rewardName} · +${current.reward.xp} XP</span></div>${ready?`<button class="btn" onclick="MeowEvents.claimSeason();renderHome()">${ko?"시즌 보상 받기":"Claim seasonal reward"}</button>`:""}${claimed()?`<div class="v428-claimed">✓ ${ko?"이번 시즌 보상 수령 완료":"Season reward claimed"}</div>`:""}</section>`;
  }
  ...
  function visitorCard(){
    ...
    return `<section class="card v428-visitor-card"><div class="v428-visitor-avatar">${visitor.icon}</div><div class="v428-visitor-copy"><div class="section-kicker">${ko?"오늘의 희귀 방문자":"Today's rare visitor"}</div><h3>${ko?visitor.nameKo:visitor.nameEn}</h3><p>${ko?visitor.lineKo:visitor.lineEn}</p><small>+${visitor.reward.xp} XP · +${visitor.reward.churu} ${ko?"츄르":"Churu"}</small></div><button class="btn ${state.claimed?"ghost":""}" ${state.claimed?"disabled":`onclick="MeowEvents.claimVisitor();renderHome()"`}>${state.claimed?(ko?"선물 받음":"Claimed"):(ko?"선물 확인":"Check gift")}</button></section>`;
  }
  ...
  function decorateHome(){
    ...
    if(!scroll)return;
    if(!scroll.querySelector(".v428-season-card")){
      const anchor=scroll.querySelector(".v427-emotion-card");
      if(anchor)anchor.insertAdjacentHTML("afterend",seasonalCard()+visitorCard());
      else scroll.insertAdjacentHTML("afterbegin",seasonalCard()+visitorCard());
    }
    if(!scroll.querySelector(".v428-collection"))scroll.insertAdjacentHTML("beforeend",collectionCard());
  }
  ...
  window.__MEOWDE_VERSION__="4.28";
  persist();
