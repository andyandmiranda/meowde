(function applyMeowdeV443SingleCompanion(){
  ...
  const VERSION="4.43-phase5-room";
  ...
  function achievementSummary(){
    ...
    const ko=isKorean();
    const percent=Math.round(count/Math.max(1,definitions.length)*100);
  ...
  function growthCard(){
    ...
    const ko=isKorean();
    const current=api.stage();
    const next=typeof api.nextStage==="function"?api.nextStage():null;
    const progress=api.evolutionProgress();
  ...
  function milestoneCard(){
    ...
    const state=api.pendingRewardUnit();
    if(!state)return "";
    const ko=isKorean();
    const reward=50;
    const name=typeof api.unitName==="function"?api.unitName(state.unit):(ko?`유닛 ${state.unit+1}`:`Unit ${state.unit+1}`);
  ...
  function persistEventState(api){
    try{localStorage.setItem("meowde-v428-events",JSON.stringify(api.state||{}))}catch(error){console.warn("Meowde event view state could not be saved:",error)}
  }
  function eventCards(){
    const api=window.MeowEvents;
    if(!api||typeof api.season!=="function"||typeof api.progress!=="function")return "";
    const ko=isKorean();
    const current=api.season();
    const value=api.progress();
    const key=`${new Date().getFullYear()}-${current.id}`;
    const claimed=Boolean(api.state&&api.state.claimed&&api.state.claimed[key]);
    const percent=Math.round(value/Math.max(1,current.goal)*100);
    const ready=value>=current.goal&&!claimed;
    ...
    if(currentVisitor){
      ...
      const visitors=api.state.visitors||(api.state.visitors={});
      const state=visitors[dateKey]||(visitors[dateKey]={seen:false,claimed:false});
    ...
    if(collectibles.length){
      ...
      collection=`<section class="card v428-collection"><div><div class="section-kicker">${ko?"한정 컬렉션":"Limited collection"}</div><h3>${ko?"시즌 기념품":"Season keepsakes"}</h3></div><div>${collectibles.map(id=>`<span title="${esc(id)}">${icons[id]||"🎖️"}</span>`).join("")}</div></section>`;
    }
  ...
  function renderCompanionHub(){
    ...
    const ko=isKorean();
    const done=Number(S.done&&S.done.length)||0;
    const streak=Number(S.streak)||0;
    const partnerLabel=ko?"나의 파트너":"My companion";
    ...
    document.documentElement.dataset.roomRenderer="canonical-v443";
  ...
  window.renderRoom=renderCompanionHub;
  window.renderMy=renderCompanionHub;
  ["renderHome","renderMap","renderReview","renderProfile","renderAchievements"].forEach(wrapRenderer);
  ...
  window.MeowSingleCompanion=Object.freeze({version:VERSION,render:renderCompanionHub,decorate,normalizeProfile,normalizeNavigation,forceSingleCompanion});
