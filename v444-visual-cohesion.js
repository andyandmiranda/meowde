(function applyMeowdeV448VisualCohesion(){
  ...
  function growthVisual(level){
    return `<svg class="v448-growth-cat" viewBox="0 0 128 174" role="img" aria-label="Meowde"><image href="${BASE_ASSET}" x="2" width="124" height="174" preserveAspectRatio="xMidYMid meet"/></svg><span class="v448-level-badge" aria-label="Level ${level}">${level}</span>`;
  }
  ...
  function refineGrowth(){
    ...
    if(title){
      const labels={
        kitten:["새싹 코더","Sprout coder"],
        junior:["꼬마 코더","Junior coder"],
        explorer:["코드 탐험가","Code explorer"],
        mentor:["코딩 메이트","Coding mate"],
        legend:["코드 마스터","Code master"]
      };
    ...
    if(copy){
      ...
      if(next){
        ...
        copy.textContent=ko()?`${left} XP 후 다음 레벨`:`${left} XP to the next level`;
      }else{
    ...
    if(xpPill){
      xpPill.classList.add("v444-xp-pill");
      xpPill.textContent=`${Math.max(0,Number(window.S&&S.xp)||0)} XP`;
    }
  }
